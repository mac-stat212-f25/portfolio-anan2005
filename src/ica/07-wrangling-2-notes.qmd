...
df_str_c <- df_dates |> 
  mutate(full_date_c = str_c(month, day, year, sep = "-"))

df_str_glue <- df_dates  |> 
  mutate(full_date_g = as.character(str_glue("{month}-{day}-{year}")))
...
df_str_sub <- df  |> 
  mutate(
    n      = nchar(word),
    start  = if_else(n %% 2 == 1, (n + 1) / 2, n / 2),
    end    = if_else(n %% 2 == 1, (n + 1) / 2, n / 2 + 1),
    middle = str_sub(word, start, end)
  )  |> 
  select(word_id, word, middle)
